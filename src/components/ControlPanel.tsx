...
export function ControlPanel({ isMicActive, toggleMic, state, disabled }: ControlPanelProps) {
    return (
        <div className="flex flex-col items-center gap-6">
            <div className="flex justify-center items-center gap-6">
                <button
                    onClick={toggleMic}
                    disabled={disabled}
                    className={`relative group w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 ${disabled
                        ? 'bg-zinc-900 border-2 border-zinc-800 text-zinc-700 cursor-not-allowed opacity-50'
                        : isMicActive
            ...
            <div className="text-center text-xs text-zinc-500 uppercase tracking-widest">
                {disabled ? 'Limit Reached' : (state === 'IDLE' ? 'Tap to Speak' : state)}
            </div>
        </div>
    );
}
